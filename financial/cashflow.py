...
class AnnualRevenue:
    ...
    def total(self) -> float:
        return self.energy_revenue + self.fcess_revenue + self.capacity_revenue + self.network_savings
...
def build_cashflow(
    ...
    for year in range(1, config.project_life_years + 1):
        ...
        energy_rev = _escalate(annual_revenue.energy_revenue, _get_rate("energy_revenue", config), year)
        fcess_rev = _escalate(annual_revenue.fcess_revenue, _get_rate("fcess_revenue", config), year)
        capacity_rev = _escalate(annual_revenue.capacity_revenue, _get_rate("capacity_revenue", config), year)
        network_sav = _escalate(annual_revenue.network_savings, _get_rate("network_savings", config), year)
        ...
        repl_capex = _escalate(annual_costs.replacement_capex, _get_rate("replacement_capex", config), year)
